...
fn eni(n: i64, exp: i64, modulus: i64) -> i64 {
    ...
    for _ in 0..exp {
        ...
        res = if res != 0 { concatenate_numbers(remainder, res) } else { remainder };
        score = remainder;
    }
    res
}
...
fn eni_part_2(n: i64, mut exp: i64, modulus: i64) -> i64 {
    ...
    res.reverse();
    res.into_iter().take(5).reduce(|res, remainder| concatenate_numbers(res, remainder)).unwrap()
}
...
fn eni_part_3(n: i64, exp: i64, modulus: i64) -> i64 {
    ...
    for _ in 0..exp {
        ...
        if let Some(cycle_start_offset) = res.iter().position(|&prev| prev == remainder) {
            ...
            let remaining_exponents = exp as usize - cycle_start_offset - cycle_length * cycle_count;
            return res.iter().take(cycle_start_offset).sum::<i64>()
                + (cycle_count as i64) * res[cycle_start_offset..].iter().sum::<i64>()
                + res[cycle_start_offset..cycle_start_offset + remaining_exponents].iter().sum::<i64>();
        }
        res.push(remainder);
        score = remainder;
    }
...
pub fn solve_part_1(input: &str) -> String {
    parse_params(input).iter().map(|p| eval(&p, &mut eni)).max().unwrap().to_string()
}

pub fn solve_part_2(input: &str) -> String {
    parse_params(input).iter().map(|p| eval(&p, &mut eni_part_2)).max().unwrap().to_string()
}

pub fn solve_part_3(input: &str) -> String {
    parse_params(input).iter().map(|p| eval(&p, &mut eni_part_3)).max().unwrap().to_string()
}
...
mod tests {
    ...
    fn test_part_one() {
        ...
        assert_eq!("11611972920", solve_part_1("A=4 B=4 C=6 X=3 Y=4 Z=5 M=11
...
A=8 B=8 C=8 X=6 Y=9 Z=6 M=16"));
    ...
    #[test]
    fn test_part_two() {
        assert_eq!("11051340", solve_part_2("A=4 B=4 C=6 X=3 Y=14 Z=15 M=11
...
A=8 B=8 C=8 X=6 Y=19 Z=16 M=16"));
        assert_eq!("1507702060886", solve_part_2("A=3657 B=3583 C=9716 X=903056852 Y=9283895500 Z=85920867478 M=188
...
A=7334 B=9016 C=8524 X=297284338 Y=1565962337 Z=86750102612 M=145"));
    ...
    #[test]
    fn test_part_three() {
        assert_eq!("1573000", solve_part_3("A=4 B=4 C=6 X=3000 Y=14000 Z=15000 M=110"));
        assert_eq!("1439940", solve_part_3("A=8 B=4 C=7 X=8000 Y=14000 Z=16000 M=120"));
        assert_eq!("2079860", solve_part_3("A=2 B=8 C=6 X=2000 Y=14000 Z=15000 M=130"));
        assert_eq!("2407850", solve_part_3("A=5 B=9 C=6 X=8000 Y=16000 Z=18000 M=140"));
        assert_eq!("2099880", solve_part_3("A=5 B=9 C=7 X=6000 Y=16000 Z=18000 M=150"));
        assert_eq!("3279640", solve_part_3("A=8 B=8 C=8 X=6000 Y=19000 Z=16000 M=160"));
    }
}