...
pub fn solve_part_2(input: &str) -> String {
    ...
    let words: Vec<String> = lines[0]["WORDS:".len()..]
        ...
        .collect();
    let ac = AhoCorasick::new(words).unwrap();
    let symbols_count: usize = lines[2..].iter().map(|line| {
        ...
        rune_intervals.iter().map(|interval| interval.upper() - interval.lower() + 1).sum::<usize>()
    }).sum();
    symbols_count.to_string()
}

fn find_words_starting_at<F>(
    rune_lines: &Vec<Vec<char>>, words: &Trie<char>, mut i: usize, mut j: usize,
    advance: F)
    -> Vec<(usize, usize)>
...
pub fn solve_part_3(input: &str) -> String {
    ...
    let rune_lines: Vec<Vec<char>> = lines[2..].iter()
    ...
    for start_i in 0..rune_lines.len() {
        for start_j in 0..width {
            ...
            find_words_starting_at(&rune_lines, &words, start_i, start_j, 
                |(i, j)| Some((i, (j + 1) % width)))
                .into_iter()
                .for_each(|c| { scales_with_runic_words.insert(c); });
            find_words_starting_at(&rune_lines, &words, start_i, start_j, 
                |(i, j)| Some((i, (j + width - 1) % width)))
                .into_iter()
                .for_each(|c| { scales_with_runic_words.insert(c); });
            find_words_starting_at(&rune_lines, &words, start_i, start_j, 
                |(i, j)| if i + 1 >= rune_lines.len() { None } else { Some((i + 1, j)) })
                .into_iter()
                .for_each(|c| { scales_with_runic_words.insert(c); });
            find_words_starting_at(&rune_lines, &words, start_i, start_j, 
                |(i, j)| if i == 0 { None } else { Some((i - 1, j)) })
                .into_iter()
                .for_each(|c| { scales_with_runic_words.insert(c); });
        }
    }
...
mod tests {
    ...
    fn test_solve_part_1() {
        assert_eq!("4", solve_part_1("WORDS:THE,OWE,MES,ROD,HER

AWAKEN THE POWER ADORNED WITH THE FLAMES BRIGHT IRE"));
        assert_eq!("3", solve_part_1("WORDS:THE,OWE,MES,ROD,HER

THE FLAME SHIELDED THE HEART OF THE KINGS"));
        assert_eq!("2", solve_part_1("WORDS:THE,OWE,MES,ROD,HER
...
THERE IS THE END"));
    ...
    fn test_solve_part_2() {
        assert_eq!("42", solve_part_2("WORDS:THE,OWE,MES,ROD,HER,QAQ
...
QAQAQ"));
    ...
    fn test_solve_part_3() {
        assert_eq!("10", solve_part_3("WORDS:THE,OWE,MES,ROD,RODEO
...
TRODEOAL"));
    ...
    fn test_solve_part_3_one_letter() {
        assert_eq!("1", solve_part_3("WORDS:H
