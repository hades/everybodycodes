...
pub fn solve_part_1(input: &str) -> String {
    ...
    loop {
        ...
        for i in 0..width {
            for j in 0..height {
                if depths[(i, j)] != level - 1 { continue; }
        ...
        if additional_amount_dug_out == 0 { break; }
            ...
            for j in 0..height {
                ...
                for check_i in ((i as isize) - 1) .. (i as isize + 2) {
                    for check_j in ((j as isize) - 1)..(j as isize + 2) {
                        if check_i < 0 || check_j < 0 { can_dig = false; continue; }
                        let check_i = check_i as usize;
                        let check_j = check_j as usize;
                        if check_i >= width || check_j >= height { can_dig = false; continue; }
